...
object Pom {
  ...
  def projectArtifactId(pomDir: File): String = {
    val pom = new File(pomDir, "pom.xml")
    val xml = XML.loadFile(pom)
    val name = (xml \\ "project" \ "artifactId").text
    name
  }

  def projectVersion(pomDir: File): String = {
    val pom = new File(pomDir, "pom.xml")
    val xml = XML.loadFile(pom)
  ...
  def scalaVersion(pomDir: File): String = {
    val pom = new File(pomDir, "pom.xml")
    val xml = XML.loadFile(pom)
    val version = (xml \\ "project" \ "properties" \ "scala.version").text
    version
  }
